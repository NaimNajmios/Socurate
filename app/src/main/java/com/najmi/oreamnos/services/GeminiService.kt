...
class GeminiService(
    ...
    companion object {
        ...
        // Moved from instance methods to Companion Object to enable unit testing and clean separation
        // Changed visibility to internal for testing access
        internal fun extractTextFromJson(root: JsonObject?): String? {
            if (root == null) return null

            // OPTIMIZATION: Try strict direct access first
            try {
                // Use safe direct access to avoid redundant has()/get() lookups
                ...
                // BUG FIX: If structure doesn't match optimized path (e.g. ClassCastException),
                // we must swallow the exception to allow the fallback to execute.
                // Previously, this exception caused the method to return null immediately.
            ...
            return try {
                ...
                Log.e(TAG, "Error extracting text from JSON (fallback)", e)
                null
            }
        }
